...
public:
	...
	std::pair<int, T>* find(const T&);
	std::pair<int, T>* find_by_rank(int);
...
template<typename T>
std::pair<int, T>* binarySearchTree<T>::find(const T& element) {
	node* rt = this ->root;
	while(rt != nullptr){
...
template<typename T>
std::pair<int, T>* binarySearchTree<T>::find_by_rank(int rank) {
	node *rt = this ->root;
	while(rt != nullptr){
		// std::cout << rt ->element.second<< ' ';
...
void binarySearchTree<T>::erase_by_rank(int rank) {
	...
	if (p != nullptr) {
		...
		// 一个孩子
			...
			if (p != this ->root) {
				if (p ->father ->rightChild == p) p ->father ->leftChild = p ->leftChild;
				else p ->father ->rightChild = p ->leftChild;
				if (p ->leftChild != nullptr) p ->leftChild ->father = p ->father;
			} else {
				this ->root = p ->leftChild;
...
int main(){
	...
	while(m--){
		...
		if (op == 0){
			...
			printf("%d ", Tr.find(x) ->first);
		} else if (op == 2) {
			Tr.erase(x);
		} else if (op == 3) {
			printf("%d ", Tr.find_by_rank(x) ->second);
		} else {
			Tr.erase_by_rank(x);
		}
	}
